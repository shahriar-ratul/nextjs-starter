...
export default function PermissionTable() {
	...
	const [selectedRow, setSelectedRow] = useState<PermissionModel | null>(null);
	...
	const onConfirm = async () => {
		...
		if (actionType === 'changeStatus') {
			try {
				const { data } = await axiosInstance.post(`/api/v1/permissions/${selectedRow?.id}/status`);
		...
		if (actionType === 'delete') {
			try {
				const { data } = await axiosInstance.delete(`/api/v1/permissions/${selectedRow?.id}`);
	...
	const columns: ColumnDef<PermissionModel>[] = [
		...
		},
		{
			...
			header: 'Slug',
			cell: ({ row }) => {
				return <div className="capitalize">{row.original.slug}</div>;
			},
		},

		{
			...
			header: 'Group',
			cell: ({ row }) => {
				return <div className="capitalize">{row.original.group}</div>;
			},
		},
		{
				...
				<div className="flex justify-center space-x-2">
					<Tooltip>
						<TooltipTrigger asChild>
							<Link
								href={`/dashboard/permissions/${row.original.id}`}
					...
					{row.original.id !== 1 && (
						<Tooltip>
							<TooltipTrigger asChild>
								<Link
									href={`/dashboard/permissions/${row.original.id}/edit`}
	...
	const fetchData = async (
		...
		const { data } = await axiosInstance.get(
			`/api/v1/permissions?page=${adjustedPage}&limit=${limit}&search=${search}&isActive=${isActive}`,
		);
	...
	const { isLoading, isError, error, isFetching, refetch } = useQuery<boolean, Error>({
		queryKey: ['permissions-list', pagination.pageIndex, pagination.pageSize, searchKey, status],
		queryFn: async () => {
			...
			setTableData(data.items as PermissionModel[]);
	...
	const table = useReactTable({
		data: tableData as PermissionModel[],
		columns: columns as ColumnDef<PermissionModel>[],
